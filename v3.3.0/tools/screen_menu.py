...
def menu_labels(lang='EN'):
    ...
    return [t['STR_MENU_WIFI'], t['STR_MENU_ONLINE'], t['STR_MENU_LOCAL'],
            t['STR_MENU_SLEEP'], t['STR_MENU_LANG'], t['STR_MENU_EXIT']]


def draw_sys_menu(d, labels, index=0, top=0):
    """drawSysMenu() aus dem Sketch, samt Scrollen ab SYS_MENU_VISIBLE."""
    frame, dim = defines.need(defines.func_consts('drawSysMenu'),
                              'frame', 'dim')

    count = len(labels)
    rows = min(count, D.SYS_MENU_VISIBLE)

    # Nachfuehren wie im Sketch: das gewaehlte Element muss sichtbar sein.
    top = min(top, index)
    if index >= top + rows:
        top = index - rows + 1
    top = max(0, min(top, count - rows))

    h = 2 * D.SYS_MENU_PAD + rows * D.SYS_MENU_LINE_H
    ...
    for r in range(rows):
        i = top + r
        ly = y + D.SYS_MENU_PAD + r * D.SYS_MENU_LINE_H
        ...
        d.draw_string(labels[i], x + D.SYS_MENU_PAD + D.SYS_MENU_MARK_W, ty)
