...
- **Wi-Fi that says what went wrong.** A failed connection is not automatically
  a bad password. The radio reads the driver's own disconnect reason and
  distinguishes a wrong password from a network that is out of range and from a
  router that never hands out an address — and it only asks you to retype the
  password when that is actually the problem.
...
| File | Size | Use with |
|---|---|---|
| `Cardputer-WebRadio_v3.1.2.bin` | 1.5 MB | SD card / M5Launcher — application only |
| `Cardputer-WebRadio_v3.1.2_merged.bin` | 4.2 MB | M5Burner, ESP Web Flasher — complete image, write to `0x0` |
...
The partition scheme is mandatory — the sketch is about 1.51 MB. PSRAM must
...
- [ESP8266Audio](https://github.com/earlephilhower/ESP8266Audio) by earlephilhower —
  the name says ESP8266, it runs on the ESP32 just as well
- M5Cardputer / M5Unified / M5GFX

**No library patch is needed any more.** Up to 3.1.1 this radio ran on
ESP32-audioI2S and required two patches: one to keep mbedTLS from eating the
heap on https redirects, one to swap the AAC decoder back to Helix. Both are
gone. ESP8266Audio brings its own Helix decoders and does not attempt https at
all.

### 3. How the audio layer is wired

The sketch still calls the ESP32-audioI2S interface. `AudioCompat.h` maps it
onto ESP8266Audio — same twelve calls, same callbacks. Two things it adds:

- **Codec, bitrate and station name.** ESP8266Audio reports none of them, so
  `connecttohost()` fetches the headers first and reads `Content-Type`,
  `icy-br` and `icy-name`. The bitrate shown in the footer is measured from
  the bytes actually flowing, not taken from the header.
- **VU levels and spectrum samples** come from `AudioVuOut`, a small output
  class of our own. It runs in the loop task, so nothing draws from a foreign
  task any more.

Going back is one line: include `<Audio.h>` instead of `"AudioCompat.h"`.
...
| | *Hold `BtnG0` during the first 2.5 s after power-on for the same reset, followed by a restart* |
...
Since 3.1.2 the question is moot: ESP8266Audio ships Helix and nothing else.
The decoder gets a fixed 90,048-byte field at compile time, so it cannot fail
for want of a contiguous block — the `Low RAM - codec too big` message from
earlier versions is gone with it. Fourteen test streams — MP3 64–320,
AAC 64–192, AAC+ 32–96 — play, and 193 station changes in a row produced no
crash and no memory loss.
