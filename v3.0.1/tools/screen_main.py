...
def show_station(d, station, title):
    ...
    d.fill_rect(0, D.HDR_TITLE_Y, 240, D.HDR_TITLE_H, BLACK)
    if title:
        d.set_font(UI_FONT_SMALL)
        d.set_text_color(WHITE, BLACK)
        d.draw_string(title, D.HDR_TEXT_X, D.HDR_TITLE_Y)
        d.set_font(UI_FONT)
...
def draw_bitrate(d, kbit):
    ...
    if kbit > 0:
        d.set_text_color(FOOTER_TEXT_COLOR, BLACK)
        d.draw_right_string('%d kbit/s' % kbit, d.width() - D.UI_MARGIN, ty)


def draw_footer(d, pct, kbit, failed):
    y = d.height() - D.FOOTER_HEIGHT
    ty = footer_text_y(d)
    ...
    if failed:
        # Auch diese Meldung steht nur im Sketch, nicht hier.
        msg, = defines.expect(defines.func_strings('drawFooter'), 1,
                              'Meldung in drawFooter()')
...
def build(args):
    ...
    show_station(d, args.station, args.title)
    show_volume(d, args.volume)
    update_battery_display(d, args.battery)
    draw_footer(d, args.wifi, args.kbit, args.failed)
