...
**Start here: [v3.1.2](v3.1.2/) — the current version.** Full documentation,
source, prebuilt firmware and the render tools are inside that folder.

![Cardputer WebRadio](v3.1.2/images/radio.png)
...
|---|---|---|
| [v3.1.2](v3.1.2/) | 11 August 2026 | **A new audio layer, and the reboots are gone.** Streams used to take the device down every few minutes — four crashes traced to corrupted lwIP structures, all in the receive path. The decoding now runs on [ESP8266Audio](https://github.com/earlephilhower/ESP8266Audio) behind a compatibility header, and both library patches are obsolete: no more mbedTLS workaround, no more AAC decoder swap. Measured afterwards: two hours of playback and 193 station changes across MP3, AAC and AAC+ without a single restart or a byte of leaked memory. The stream title moved out of the header into the empty display where there is room for it, in Noto Sans with umlauts and accents, wrapped and standing still instead of scrolling. Wi-Fi failures still say what they were, a lost stream reconnects by itself, and the Wi-Fi comes back on its own after the access point disappears. |
...
decoding since 3.1.2, and **schreibfaul1**'s
[ESP32-audioI2S](https://github.com/schreibfaul1/ESP32-audioI2S) before that. Full credits in each version's README.

## License

MIT — see [LICENSE](v3.1.2/LICENSE).
