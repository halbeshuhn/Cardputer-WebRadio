...
// Eine Listenzeile auffrischen, ohne sie vorher zu leeren.
//
// Frueher stand vor jedem Neuzeichnen ein fillRect ueber die ganze Zeile. Das
// Display hat keinen Zwischenspeicher: zwischen dem Leeren und dem Text ist
// die Zeile fuer einen Moment leer, und genau diesen Zustand erwischt der
// Bildaufbau - als Flackern.
//
// Noetig ist das Leeren nicht: Font0 malt bei deckender Textfarbe jede
// Zeichenzelle voll aus, 9x12 Pixel ohne Luecke (M5GFX, GLCDfont::drawChar).
// Der Text raeumt seinen Untergrund selbst weg, waehrend er entsteht. Deckend
// ist er hier immer - der Sketch setzt durchweg setTextColor(farbe, TFT_BLACK),
// und die Voreinstellung von M5GFX ist es ebenfalls.
//
// Zu raeumen bleibt nur, was links und rechts vom Text ueberhaengt: beim
// Durchlaufen wandert er ueber x=0 hinaus, ohne diese Streifen blieben beim
// Ruecksprung ein paar Pixel stehen.
static void wifiDrawScrollLine(const char* text, int x, int y, int w) {
    M5Cardputer.Display.drawString(text, x, y);

    if (x > 0) {
        M5Cardputer.Display.fillRect(0, y, x, 18, BLACK);
    }
    int end = x + w;
    if (end < 240) {
        M5Cardputer.Display.fillRect(end, y, 240 - end, 18, BLACK);
    }
}
...
// Fest belegt statt std::vector. Der Vektor holte seinen Block beim ersten
// Scan - also spaet, mitten im Betrieb - und gab ihn wegen clear() ohne
// shrink_to_fit() nie wieder her: ein dauerhaftes Loch im Heap an der
// schlechtesten denkbaren Stelle. mbedTLS braucht zwei Bloecke von je gut
// 16 KB am Stueck; genau solche Loecher zerschneiden sie.
...
enum WifiFail : uint8_t {
    ...
    WIFI_FAIL_NO_START,  // WiFi.begin() ist gar nicht erst angelaufen
...
// Nicht vom Ereignishandler, sondern von connectOnce() gesetzt: WiFi.begin()
// hat WL_CONNECT_FAILED geliefert, der Versuch hat also nie begonnen. Braucht
// ein eigenes Fach, weil in diesem Fall gar kein Ereignis kommt und die
// Auswertung unten sonst auf WIFI_FAIL_TIMEOUT lauefe - also auf "Router
// antwortet nicht", obwohl der Router nie gefragt wurde.
static bool staNoStart = false;
...
static WifiFail classifyFailure() {
    // Zuerst, denn dieser Fall hat keinen Grundcode und keine Ereignisse.
    if (staNoStart) return WIFI_FAIL_NO_START;
...
static bool connectOnce(const String& ssid, const String& pass,
    ...
    // Der Rueckgabewert gehoert geprueft. WiFiSTAClass::begin() liefert
    // WL_CONNECT_FAILED, sobald schon STA.begin() oder STA.connect()
    // scheitert - unter anderem bei einem Fehler aus esp_wifi_set_config()
    // (Kern 3.3.8, STA.cpp). Dann laeuft nichts an, es kommt kein einziges
    // Ereignis, und die Warteschleife unten wuerde 15 s lang Punkte malen und
    // danach "No answer from the network" melden. Genau darauf ist die
    // Fehlersuche zu Meldung #2 hereingefallen.
    if (WiFi.begin(ssid.c_str(), pass.c_str(), channel, bssid)
            == WL_CONNECT_FAILED) {
        staNoStart = true;
        return false;
    }
...
bool fastConnect(const String& ssid, const String& pass,
    ...
    for (int attempt = 0; attempt < 2; attempt++) {
        ...
        // Ist der Versuch gar nicht angelaufen, liegt es nicht an Kanal oder
        // BSSID; der zweite Anlauf scheitert an derselben Stelle.
        if (lastFail == WIFI_FAIL_NO_START) break;
...
static bool showWifiFailure(bool savedCreds, unsigned long ms) {
    ...
    switch (lastFail) {
        ...
        case WIFI_FAIL_NO_START:
            // Bewusst ohne Schuldzuweisung an den Launcher: der ist der
            // bekannte Fall, aber nicht der einzig moegliche. Wichtig ist,
            // dass hier nicht mehr der Router beschuldigt wird.
            l1 = "Could not start";
            l2 = "the WiFi driver";
            break;
...
int selectSavedToReplace(const char* title = "Replace WiFi:",
    ...
    while (true) {
        ...
        for (int i = 0; i < savedCount; i++) {
            ...
            wifiDrawScrollLine(line.c_str(), x, y, w);
        }

        // Der Hinweis steht fest; deckend gezeichnet ueberschreibt er sich
        // selbst deckungsgleich. Das fillRect davor hat nur geflackert.
...
String scanAndDisplayNetworks() {
    ...
    networkCount = 0;
    for (int i = 0; i < scanResult && networkCount < MAX_NETWORKS; i++) {
        String ssid = WiFi.SSID(i);
        if (ssid.length() == 0 || ssid.length() > 32) continue;

        WiFiNetwork& net = networks[networkCount];
        net.ssid       = ssid;
    ...
    std::sort(networks, networks + networkCount,
    ...
    while (true) {
        ...
        for (int i = 0; i < networkCount; i++) {
            ...
            // Fester Puffer auf dem Stapel, kein String. Diese Schleife
            // laeuft in der Zeichenschleife darueber staendig durch: mit
            // String waren es je Durchgang bis zu zehn Anforderungen von
            // 112 Bytes und ebenso viele Freigaben - waehrend der Ton laeuft.
            // Genau diese kleinen, kurzlebigen Bloecke zerschneiden den Heap.
            char line[128];
            snprintf(line, sizeof(line), "%s%s (%ddBm)%s",
                     selected ? "-> " : "   ",
                     networks[i].ssid.c_str(),
                     (int)networks[i].rssi,
                     networks[i].encryption != WIFI_AUTH_OPEN ? " *" : "");
            ...
            wifiDrawScrollLine(line, x, y, textWidth);
        }
...
// ---------------------------------------------------------------------------
// Eigener Wiederverbinder
//
// Am 10.8.2026 gemessen: verschwindet der Zugangspunkt (Grundcode 37,
// END_BA), versucht der Treiber es einmal, faellt auf 202 (AUTH_FAIL) und
// gibt danach auf - obwohl setAutoReconnect(true) gesetzt ist. Das WLAN kam
// nicht von selbst zurueck.
//
// Diese Funktion laeuft aus loop() und blockiert nicht: sie stoesst nur alle
// WIFI_KEEPALIVE_MS einen neuen Versuch an und sieht beim naechsten Durchlauf
// nach. Die Wiedergabe darf dabei nicht stehenbleiben.
// ---------------------------------------------------------------------------
#define WIFI_KEEPALIVE_MS 15000

void wifiKeepAlive() {
    static unsigned long nextTry = 0;

    // Die Masken aus dem Systemmenue haben ihre eigene Schleife und ihr
    // eigenes Verbindungsverhalten - da nicht hineinfunken.
    if (wifiAbortEnabled) return;
    ...
    if (nextTry == 0) {
        nextTry = millis() + WIFI_KEEPALIVE_MS;
        return;
    }
    if ((long)(millis() - nextTry) < 0) return;
    nextTry = millis() + WIFI_KEEPALIVE_MS;

    // Zugang des zuletzt benutzten Platzes nehmen. Die Liste steht seit dem
    // Start im RAM; ohne Eintrag gibt es nichts zu versuchen.
    ...
    // Kein wifioff, kein Erase: das Funkmodul soll an bleiben, und der
    // geteilte Treibereintrag geht uns nichts an.
    WiFi.disconnect(false, false);
    WiFi.begin(saved[li].ssid.c_str(), saved[li].pass.c_str());
}

void connectToWiFi() {

    // Muss vor dem allerersten WLAN-Aufruf des Programms stehen, und das ist
    // diese Funktion: der Merker wird nur ein einziges Mal ausgewertet, naemlich
    // in wifiLowLevelInit() beim ersten Anfassen des Funkmoduls (Kern 3.3.8,
    // WiFiGeneric.cpp). Spaeter gesetzt wirkt er nicht mehr.
    //
    // Was er bewirkt: der Kern ruft esp_wifi_set_storage(WIFI_STORAGE_RAM),
    // und der Treiber schreibt seine Konfiguration beim Verbinden nicht mehr
    // in den NVS-Bereich, den sich alle Programme auf dem Geraet teilen. Wir
    // brauchen das nicht - die Zugangsdaten legen wir selbst ueber Preferences
    // ab. Anlass war Meldung #2: unter M5Launcher bis 2.8.0 ist dieser Bereich
    // kleiner als ueblich (0x4000 statt 0x5000), und dort schlug das Verbinden
    // bei drei Benutzern fehl. So haengt das Radio gar nicht mehr davon ab.
