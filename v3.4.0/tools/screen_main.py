...
def show_volume(d, volume):
    # Die beiden Toene stehen in volDrawScale() im Sketch - dort, wo die
    # Skala seit dem Puls beim Regeln gemalt wird.
    col_on, col_grid = defines.need(defines.func_consts('volDrawScale'),
                                    'colOn', 'colGrid')
...
# ---------------------------------------------------------------------------
    ...
    draw_batt_pct(d, level)


def draw_batt_pct(d, level):
    """Der Ladestand als Zahl, ohne Einheit, rechtsbuendig BATT_PCT_GAP vor
    dem Gehaeuse - siehe updateBatteryDisplay() im Sketch."""
    total = int(D.BATT_W) + int(D.BATT_GAP) + int(D.BATT_TIP_W)
    batt_x = 240 - int(D.UI_MARGIN) - total

    d.set_font('Font0')
    d.set_text_size(1)
    d.set_text_color(FOOTER_TEXT_COLOR, BLACK)
    t = str(level)
    d.draw_string(t, batt_x - int(D.BATT_PCT_GAP) - d.text_width(t),
                  int(D.BATT_PCT_Y))
    d.set_font('NokiaFC')
...
def draw_footer(d, pct, kbit, failed, lowram=False, codec='MP3',
                playlist=False, hls=False, https=False, sleep=None,
                left=None, stalled=False):
    y = d.height() - D.FOOTER_HEIGHT
    ty = footer_text_y(d)
    ...
    if failed:
        # Fuenf Meldungen, in der Reihenfolge des Quelltextes: zu wenig
        # Speicher, HLS, sonstige Wiedergabeliste, versiegter Sender,
        # unerreichbarer Sender.
        #
        # Die Zahl hat sich zweimal geaendert: die https-Meldung fiel weg, als
        # am 19.8.2026 echtes TLS kam (der Schalter https= bleibt aus
        # Vertraeglichkeitsgruenden stehen und zeigt nichts mehr), und am
        # 22.8.2026 kam "Stream stopped" dazu - der Sender lief und hoerte auf
        # zu liefern, was etwas anderes ist als einer, der nie antwortet.
        lowmem, hlsmsg, liste, versiegt, tot = defines.expect(
            defines.func_strings('drawFooter'), 5, 'Meldungen in drawFooter()')
        msg = tot
        if lowram:     msg = lowmem
        elif hls:      msg = hlsmsg
        elif playlist: msg = liste
        elif stalled:  msg = versiegt
        d.set_text_color(ORANGE, BLACK)
        d.draw_centre_string(msg, 120, ty)
    else:
        # drawSleepOrName(): laeuft ein Sleeptimer, steht links die Restzeit
        # in Gelb statt der Kennung. Im Equalizer steht dort die Vordaempfung
        # - die kommt als left= herein und geht beidem vor.
        if left:
            d.set_text_color(FOOTER_TEXT_COLOR, BLACK)
            d.draw_string(left, D.UI_MARGIN, ty)
        elif sleep:
            d.set_text_color(YELLOW, BLACK)
            d.draw_string(sleep + ' .zZ', D.UI_MARGIN, ty)
        else:
            d.set_text_color(FOOTER_TEXT_COLOR, BLACK)
...
def _band(d):
    """Der Bereich zwischen roter Linie und Fusszeile, wie header_height im
    Sketch: HDR_RULE_Y + 1 bis 135 - FOOTER_HEIGHT."""
    y = int(D.HDR_RULE_Y) + 1
    return y, d.height() - y - int(D.FOOTER_HEIGHT)


def _bild_laden(pfad):
    """BMP von sips lesen - Zeilen von oben nach unten, 24 oder 32 Bit."""
    import struct
    b = open(pfad, 'rb').read()
    ...
    for y in range(abs(h)):
        for x in range(w):
            i = off + y * zl + x * n
            px[(x, y)] = (rgb(b[i+3], b[i+2], b[i+1]) if bits == 32
                          else rgb(b[i+2], b[i+1], b[i]))
    return w, abs(h), px


def draw_info_screen(d, zeilen, logo=None):
    """drawInfoScreen(): Logo links im festen Quadrat, vier Zeilen rechts."""
    by, bh = _band(d)
    d.fill_rect(0, by, 240, bh, BLACK)

    lx = int(D.UI_MARGIN)
    ly = by + (bh - int(D.INFO_LOGO)) // 2
    seite = int(D.INFO_LOGO)

    d.draw_rect(lx, ly, seite, seite, DARKGREY)
    if logo:
        w, h, px = _bild_laden(logo)
        ox, oy = lx + (seite - w) // 2, ly + (seite - h) // 2
        for y in range(min(h, seite)):
            for x in range(min(w, seite)):
                d.draw_pixel(ox + x, oy + y, px[(x, y)])

    d.set_font(TITLE_FONT)
    d.set_text_color(FOOTER_TEXT_COLOR, BLACK)
    tx = lx + seite + int(D.INFO_GAP)
    maxw = 240 - int(D.UI_MARGIN) - tx
    lh = int(D.TITLE_LINE_H)
    ty = by + (bh - int(D.INFO_LINES) * lh) // 2

    for i, z in enumerate(zeilen[:int(D.INFO_LINES)]):
        while z and d.text_width(z) > maxw:      # abschneiden wie im Sketch
            z = z[:-1]
        d.draw_string(z, tx, ty + i * lh)
...
def build(args):
    ...
    draw_footer(d, args.wifi, args.kbit, args.failed, args.lowram, args.codec,
                args.playlist, args.hls, args.https, getattr(args, 'sleep', None),
                None, getattr(args, 'stalled', False))
    ...
    if args.vis in ('vu', 'vupeak'):
        ...
        draw_info_screen(d, getattr(args, 'info', None) or [], getattr(args, 'logo', None))
    else:
...
def main():
    ...
    p.add_argument('--playlist', action='store_true', help='mit --failed: unaufloesbare Wiedergabeliste')
    p.add_argument('--hls', action='store_true', help='mit --failed: HLS-Haeppchenliste')
    p.add_argument('--stalled', action='store_true',
                   help='mit --failed: Sender lief und versiegte')
    p.add_argument('--https', action='store_true', help='mit --failed: https auch ueber http tot')
    p.add_argument('--sleep', default=None, help='Restzeit des Sleeptimers, etwa 01:27')
    p.add_argument('--vis', choices=('vu', 'vupeak', 'spectrum', 'off', 'info'),
                   default='vu')
    p.add_argument('--logo', default=None, help='BMP fuer den Info-Schirm, 76x76')
    p.add_argument('--info', nargs='*', default=None, help='die vier Zeilen')
