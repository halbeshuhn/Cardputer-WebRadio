...
def load_nokia_fonts(path=None):
    """NokiaFC und NokiaFCSmall aus NokiaFC.h lesen."""
    path = path or os.path.join(SKETCH_DIR, 'NokiaFC.h')
    with open(path, encoding='utf-8', errors='replace') as f:
        text = f.read()

    fonts = {}
    for name in ('NokiaFC', 'NokiaFCSmall'):
...
class Display:
    ...
    def __init__(self, width=WIDTH, height=HEIGHT):
        ...
        nokia = load_nokia_fonts()
        self.fonts = dict(nokia)
    ...
    def set_text_size(self, n):
        self.size_x = self.size_y = int(n)

    def font_height(self):
        return self.font.height * self.size_y
    ...
    def _draw_char_glcd(self, code, x, y):
        ...
        for i in range(font.datawidth):
            ...
            for j in range(font.height):
                on = bool(col & (1 << j))
                if on or fillbg:
                    self.fill_rect(x + i * sx, y + j * sy, sx, sy,
                                   self.fore if on else self.back)
        if fillbg:
            self.fill_rect(x + font.datawidth * sx, y,
                           (font.width - font.datawidth) * sx,
                           font.height * sy, self.back)
        return font.width * sx
